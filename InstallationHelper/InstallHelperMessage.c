//
//  InstallHelperMessage.c
//  Sparkle
//
//  Created by Volodimir Moskaliuk on 1/23/18.
//

#include "InstallHelperMessage.h"
#include <errno.h>
#include <poll.h>
#include <syslog.h>
#include <unistd.h>
...
int sendMessage(const struct SUHelperMessage * messageOut, struct SUHelperMessage * messageIn) {
	...
	size_t bytesCount = messageSize(messageOut);
	size_t bytesWritten = write(socket_fd, messageOut, bytesCount);
	if(bytesCount != bytesWritten) {
		syslog(LOG_ERR, "tried to write %zu, but wrote %zu", bytesCount, bytesWritten);
		close(socket_fd);
		return 1;
	}
...
{
	struct SUHelperMessage messageOut, messageIn;
	initMessage(messageOut, SUM_Version)
